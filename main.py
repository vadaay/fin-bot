...
def get_news_bitsmedia():
    try:
        ...
        response = requests.get(url, headers=headers, timeout=10)
        import re
        titles = re.findall(r'<a class="news-item__title"[^>]*href="([^"]*)"[^>]*>(.*?)</a>', response.text)
        news = []
...
def get_news_api():
    ...
    params = {
        "q": "биткоин OR криптовалюта OR блокчейн OR ставка OR инфляция",
        "language": "ru",
        "sortBy": "publishedAt",
        "pageSize": 5,
        "apiKey": api_key
...
def get_all_news():
    ...
    all_news = []
    all_news.extend(get_rss_news_rbc())
    all_news.extend(get_rss_news_forklog())
    all_news.extend(get_news_bitsmedia())
    all_news.extend(get_news_coindesk())
    all_news.extend(get_news_api())
...
def create_price_chart(coin_id, coin_name, color):
    try:
        ...
        chart_config = {
            ...
            "options": {
                "plugins": {"title": {"display": True, "text": f"{coin_name} за 7 дней", "color": "#fff", "font": {"size": 18}}, "legend": {"display": False}},
                "scales": {"y": {"ticks": {"color": "#ccc", "callback": "function(v){return '$'+v.toLocaleString()}"}, "grid": {"color": "#333"}},
...
def send_post():
    ...
    t = datetime.now()
    d, tm = t.strftime('%d.%m.%Y'), t.strftime('%H:%M МСК')
    
    # Пост 1: Индекс страха
    if fg:
        caption = f"📊 <b>Крипто-рынок • {d}</b>\n🕐 {tm}\n\n🎭 <b>Индекс страха:</b> {fg['value']}/100 ({fg['value_classification']})\n🌡 <b>Настроение:</b> {get_market_sentiment(btc_ch)}"
        gauge = create_fear_greed_gauge(int(fg["value"]), fg["value_classification"])
        if gauge:
            send_telegram_photo(token, channel, gauge, caption)
        else:
            send_telegram_message(token, channel, caption)
    ...
    if all_news:
        ...
        for news in all_news[:5]:
            ...
            img = news.get("image", "")
            if img:
                send_telegram_photo(token, channel, img, caption)
            else:
                send_telegram_message(token, channel, caption)
    else:
        send_telegram_message(token, channel, "📰 Нет свежих новостей")
